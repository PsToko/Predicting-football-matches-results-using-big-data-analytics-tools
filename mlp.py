...
class ModelManager:
    ...
    def background_train(self):
        try:
            ...
            class_weights_tensor = torch.FloatTensor(class_weights).to(X_train_tensor.device)

            # Initialize model
            input_size = X_train.shape[1]
            print(f"\nA {input_size}")
            self.model = FootballPredictor(input_size)
            ...
            self.model.eval()
            with torch.no_grad():
                y_pred = self.model(X_test_tensor).argmax(dim=1).numpy()
            ...
            for i in range(X_test_tensor.shape[1]):
                X_shuffled = X_test_tensor.clone()
                idx = torch.randperm(X_shuffled.shape[0])
                X_shuffled[:, i] = X_shuffled[idx, i]

                with torch.no_grad():
                    y_shuffled_pred = self.model(X_shuffled).argmax(dim=1)
                    acc = accuracy_score(y_test_tensor.numpy(), y_shuffled_pred.numpy())

                importance = accuracy_score(y_true, y_pred) - acc
                importances.append((self.feature_names[i], importance))
    ...
    def predict(self, home_team, away_team):
        ...
        try:
            ...
            self.model.eval()
            with torch.no_grad():
                probs = self.model(features_tensor).numpy()[0]
...
class FootballPredictorApp:
    ...
    def setup_ui(self):
        self.root.title("Football Match Predictor (MLP)")
