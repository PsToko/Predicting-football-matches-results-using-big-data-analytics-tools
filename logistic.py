...
class ModelManager:
    ...
    def background_train(self):
        try:
            print("Starting training with Multinomial Logistic Regression...")

            sample = matches_df.sample(fraction=0.1).collect()
            X, y = [], []
...
class FootballPredictorApp:
    ...
    def setup_ui(self):
        self.root.title("Football Match Predictor (Logistic Regression)")
